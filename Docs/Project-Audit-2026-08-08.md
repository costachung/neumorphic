...
项目核心代码规模小、没有第三方依赖，当前 macOS/iOS 可以编译；但平台声明、测试、Swift 6 兼容性、公共 API 和无障碍规范存在明显缺口。在处理高优先级问题前，不建议发布新版本。
...
- 原占位测试已替换为颜色模式往返和默认颜色解析测试。
- 已移除旧的 `Tests/LinuxMain.swift` 与 `XCTestManifests.swift`，改用自动发现；当前 `swift test` 执行 4 个测试，全部通过，包含公共样式入口、disabled 状态和负几何参数边界测试。

### 3. 缺少 CI。**状态：已完成。**

- 已新增 `.github/workflows/ci.yml`，包含格式检查、macOS `swift test` 和 iOS SDK 构建。
- 已补充 Swift 6 严格检查和基于 `v2.0.7` 的 API 兼容性门禁。
...
- `Package.swift` 已调整为 Swift tools 5.3。
- `FixedSizeSoftDynamicButtonStyle.swift` 使用了 Swift 5.3 才引入的多尾随闭包语法。
- 应先确定兼容策略，再把最低版本设为真实可编译的版本；现代化方案可考虑 tools 6.x 和 Swift 6 严格并发。
...
- 所有公开类型和方法应补充 `///` 文档注释。
- 已增加 DocC catalog、公开类型/成员注释和 API 使用入口。
- CI 已增加 `swift package diagnose-api-breaking-changes v2.0.7`，防止意外破坏公共 API。
- 已补充 `CHANGELOG.md`、`CONTRIBUTING.md`、`SECURITY.md` 和工具链支持说明。

## P3：维护性现代化

- README 的平台要求已与 Package 声明同步。
- README 安装示例已更新为 `2.0.7`。
- 示例工程已将 Swift 版本从 5.0 调整为 5.3，并增加 iOS 16+ `NavigationStack`、iOS 14 `NavigationView` fallback；iOS 示例已迁移到 SwiftUI `App` 生命周期并移除过时的 armv7 能力声明。
- 已清理本次涉及代码中的冗余 `return`/`self`、行尾空白和多余空行；更大范围的格式化可按 `.swift-format` 配置分批执行。
- 已增加 `.swift-format` 统一配置；CI 目前继续使用 `git diff --check`，格式化命令可由开发环境执行。

## 建议执行顺序

1. 撤销 tvOS 支持声明，并同步 README。**已完成；README 原已仅列出 iOS/macOS。**
2. 建立 iOS/macOS CI，补充非空测试。
3. 解决 `SwitchToggleStyle` 命名冲突。**已完成；需在 major 版本发布。**
4. 解决 Swift 6 全局可变状态问题。
5. 完成无障碍修正。**交互、对比度、状态语义和命中区域已完成。**
6. 确定最低工具链策略，清理旧测试清单和示例代码。
7. 补齐公开成员文档、贡献规范、变更日志和 API 兼容检查。**基础 DocC catalog 已完成。**
